...
function alerts(ms,cats,a){ const realized=ms.filter(m=>!isProj(m)); const base=realized.length?realized:ms; const worst=base.reduce((x,m)=>DATA.monthly[m].resultado<DATA.monthly[x].resultado?m:x,base[0]); const maxOut=ms.reduce((x,m)=>DATA.monthly[m].saidas>DATA.monthly[x].saidas?m:x,ms[0]); const top=cats[0]; const avgOut=ms.reduce((s,m)=>s+DATA.monthly[m].saidas,0)/Math.max(ms.length,1); return [`<div class="pdf-alert" style="--alert:${DATA.monthly[worst].resultado<0?'#F87171':'#20D39B'}"><b>Pior resultado observado</b><span>${mLong(worst)}: ${DATA.monthly[worst].resultado>=0?'+':'-'}${moneyFull(Math.abs(DATA.monthly[worst].resultado))}.</span></div>`,`<div class="pdf-alert" style="--alert:#F59E0B"><b>Maior desembolso mensal</b><span>${mLong(maxOut)} concentrou ${moneyFull(DATA.monthly[maxOut].saidas)} em saídas gerenciais. Média do recorte: ${moneyFull(avgOut)}.</span></div>`,`<div class="pdf-alert" style="--alert:#22D3EE"><b>Categoria dominante</b><span>${top?`${esc(top.name)} representa ${percent(top.pct)} das saídas do período (${moneyFull(top.value)}).`:'Sem categoria dominante no período.'}</span></div>`].join(''); }
  ...
  /* ============================================================
     ...
     Constrói UM relatório paginado A4 na ordem:
       1. Capa Diretoria (veredito + selo + caixa gerado)
       2. Fluxo de Caixa
       3. Custos Fixos
       4. DRE assinada (Priori)
       5. Metodologia (caixa Bling × competência Priori)
  ...
  function buildCouncilReport(){
    ...
    report.innerHTML = [
      // 1 · Capa Diretoria
      page('Pacote do Conselho · Capa',1,T,`<div class="pdf-content-fill"><div style="height:6mm"></div><div class="pdf-badge">PERFORMANCE FINANCEIRA · ${esc(realizedLabel)}</div><h1 class="pdf-title" style="font-size:34pt;margin-top:7mm;max-width:170mm">Pacote do Conselho<br>Performance Financeira 2026</h1><p class="pdf-subtitle">Documento executivo consolidado para o conselho. Reúne fluxo de caixa, custos fixos e DRE assinada (regime de competência) em um único relatório paginado.</p><div class="pdf-card" style="margin-top:6mm;border-left:3px solid var(--accent, #34D399)"><div class="pdf-label">Veredito do período</div><div class="pdf-value ${verdict.tone}">${esc(verdict.word)}</div><div class="pdf-note">Caixa operacional gerado (acum. realizado, exclui antecipações/empréstimos): <b style="color:${op.accum>=0?'#20D39B':'#F87171'}">${op.accum>=0?'+':'-'}${moneyFull(Math.abs(op.accum))}</b>. Resultado contábil assinado (acum.): <b style="color:${lucro>=0?'#20D39B':'#F87171'}">${lucro>=0?'+':'-'}${moneyFull(Math.abs(lucro))}</b>.</div></div><div class="pdf-grid-3" style="margin-top:6mm"><div class="pdf-card"><div class="pdf-label">Entradas (caixa realizado)</div><div class="pdf-value gold">${moneyFull(aReal.entradas)}</div></div><div class="pdf-card"><div class="pdf-label">Saídas gerenciais</div><div class="pdf-value">${moneyFull(aReal.saidas)}</div></div><div class="pdf-card"><div class="pdf-label">Resultado de caixa (realizado)</div><div class="pdf-value ${(aReal.entradas-aReal.saidas)>=0?'gold':'red'}">${(aReal.entradas-aReal.saidas)>=0?'+':'-'}${moneyFull(Math.abs(aReal.entradas-aReal.saidas))}</div></div></div></div>`),
      // 2 · Fluxo de Caixa
